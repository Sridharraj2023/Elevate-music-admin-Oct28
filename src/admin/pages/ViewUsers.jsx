...
function ViewUsers() {
  ...
  const filteredUsers = users.filter(user => {
    ...
    // Status filter
    if (statusFilter !== 'all') {
      if (statusFilter === 'active' && user.subscriptionStatus !== 'Active') return false;
      if (statusFilter === 'expired' && user.subscriptionStatus !== 'Expired') return false;
      if (statusFilter === 'inactive' && user.subscriptionStatus !== 'No Subscription') return false;
      if (statusFilter === 'expiring' && (user.subscriptionStatus !== 'Active' || user.daysRemaining > 7)) return false;
      if (statusFilter === 'canceled' && user.subscriptionStatus !== 'Canceled') return false;
    }
  ...
  const handleSort = (type) => {
    ...
    switch (type) {
      case 'name':
        newSortOption = sortOption === 'alphabetical-asc' ? 'alphabetical-desc' : 'alphabetical-asc';
        break;
      case 'email':
        newSortOption = sortOption === 'alphabetical-asc' ? 'alphabetical-desc' : 'alphabetical-asc';
        break;
  ...
  const getStatusBadge = (status, daysRemaining) => {
    if (status === 'Active') {
      if (daysRemaining <= 7) {
        return <span className="badge badge-warning">Expiring ({daysRemaining}d)</span>;
      }
  ...
  const stats = {
    ...
    expiring: users.filter(u => u.subscriptionStatus === 'Active' && u.daysRemaining <= 7).length
  };
  ...
  return (
    <div className="card">
      ...
      <div className="search-sort-container" style={{ display: 'flex', gap: '15px', marginBottom: '20px', alignItems: 'center', flexWrap: 'wrap' }}>
        <div className="search-container" style={{ position: 'relative', flex: '1', minWidth: '250px' }}>
          <input
            type="text"
              ...
              <thead>
                <tr>
                  <th className="sortable" onClick={() => handleSort('name')}>
                    Name {sortOption === 'alphabetical-asc' && '↑'} {sortOption === 'alphabetical-desc' && '↓'}
                  </th>
                  <th className="sortable" onClick={() => handleSort('email')}>
                    Email {sortOption === 'alphabetical-asc' && '↑'} {sortOption === 'alphabetical-desc' && '↓'}
                  </th>
                  <th>Status</th>
                  <th>Payment Date</th>
                  ...
                  <th className="sortable" onClick={() => handleSort('newest')}>
                    Created {sortOption === 'newest' && '↓'} {sortOption === 'oldest' && '↑'}
                  </th>
                  <th>Actions</th>
                </tr>
              </thead>
                ...
                {currentUsers.map(user => (
                  <tr key={user._id} className={!user.isActive ? 'inactive-user' : ''}>
                    ...
                    <td className="conditional-column">
                      {user.subscriptionStatus === 'No Subscription' ? '' : formatDate(user.expiryDate)}
                    </td>
                    <td className="conditional-column">
                      {user.subscriptionStatus === 'No Subscription' ? '' : 
                        (user.daysRemaining > 0 ? `${user.daysRemaining} days` : 'N/A')}
                    </td>
                    <td>
                      {user.autoDebit ? 
          ...
          <div className="modal-content" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="user-details">
              ...
              <div className="detail-section">
                ...
                <p><strong>Status:</strong> {getStatusBadge(selectedUser.subscriptionStatus, selectedUser.daysRemaining)}</p>
                ...
                <p><strong>Days Remaining:</strong> {selectedUser.daysRemaining > 0 ? `${selectedUser.daysRemaining} days` : 'N/A'}</p>
                <p><strong>Interval:</strong> {selectedUser.subscription?.interval || 'N/A'}</p>
                <p><strong>Auto-Debit:</strong> {selectedUser.autoDebit ? '✅ Enabled' : '❌ Disabled'}</p>
                <p><strong>Cancel at Period End:</strong> {selectedUser.subscription?.cancelAtPeriodEnd ? 'Yes' : 'No'}</p>
              </div>
              
              <div className="detail-section">
                <h4>Notification Preferences</h4>
                <p><strong>Email Reminders:</strong> {selectedUser.notificationPreferences?.emailReminders ? '✅ Enabled' : '❌ Disabled'}</p>
                <p><strong>Push Notifications:</strong> {selectedUser.notificationPreferences?.pushNotifications ? '✅ Enabled' : '❌ Disabled'}</p>
                ...
                <p><strong>Last Reminder Sent:</strong> {formatDate(selectedUser.notificationPreferences?.lastReminderSent)}</p>
              </div>
            </div>
            <div className="modal-footer">
              <button className="btn btn-secondary modal-close-btn" onClick={() => setShowModal(false)}>
                Close
              </button>
            </div>
          </div>
